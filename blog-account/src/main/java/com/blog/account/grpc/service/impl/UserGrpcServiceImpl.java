...
public class UserGrpcServiceImpl implements UserGrpcService {
  ...
  public UserRegistrationResponse createUser(UserRegistrationRequest request) {
    ...
    if (!validateSuccess) {
      return res.setSuccess(false).build();
    }
    ...
    return UserRegistrationResponse.newBuilder()
        .setSuccess(true)
        .setData(IdResponse.newBuilder().setId(savedUser.getUserId().toString()).build())
        .build();
  }
  ...
  public ListUserResponse listUser(ListUserRequest request) {
    ...
    return ListUserResponse.newBuilder()
        .setSuccess(true)
        .setData(
            ListUserResponse.Data.newBuilder()
                .addAllUsers(userMapper.toGrpc(pageUser.getItems()))
                .setPageable(
                    PageableUtils.normalize(request.getPageable(), pageUser.getTotalElement()))
                .build())
        .build();
  }
  ...
  public GetUserByIdResponse getUserById(IdRequest request) {
    var user =
        userRepository
            ...
            .orElseThrow(
                () -> new GrpcNotFoundException("User not found with id " + request.getId()));
    return GetUserByIdResponse.newBuilder()
        .setData(GetUserByIdResponse.Data.newBuilder().setUser(userMapper.toGrpc(user)).build())
        .build();
  }
}
