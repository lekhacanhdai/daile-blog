...
public class UserDslRepository {
  ...
  public Page<UserEntity> listUser(ListUserRequest request) {
    ...
    if (request.getPageable().getSort().equalsIgnoreCase("username")) {
      query.orderBy(
          new OrderSpecifier<>(order, user.username, OrderSpecifier.NullHandling.NullsFirst));
    } else if (request.getPageable().getSort().equalsIgnoreCase("fullName")) {
      query.orderBy(
          new OrderSpecifier<>(order, user.fullName, OrderSpecifier.NullHandling.NullsFirst));
    } else if (request.getPageable().getSort().equalsIgnoreCase("email")) {
      query.orderBy(
          new OrderSpecifier<>(order, user.email, OrderSpecifier.NullHandling.NullsFirst));
    } else {
      query.orderBy(
          new OrderSpecifier<>(order, user.createdDate, OrderSpecifier.NullHandling.NullsFirst));
    }
