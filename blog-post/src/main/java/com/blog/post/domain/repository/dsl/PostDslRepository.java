...
public class PostDslRepository {
    ...
    private final static QPostEntity post = QPostEntity.postEntity;
    private final static QPostTagEntity postTag = QPostTagEntity.postTagEntity;
    ...
    public Page<PostEntity> listPost(MListPostRequest request) {
        ...
        if (StringUtils.isNoneBlank(request.getSearchTerm())) {
            query.where(post.title.containsIgnoreCase(request.getSearchTerm())
        ...
        if (!request.getTagIdsList().isEmpty()) {
            query.where(
                    queryFactory.selectZero()
                            .from(postTag)
                            .where(postTag.post.postId.eq(post.postId)
                                    .and(postTag.tag.tagId.in(request.getTagIdsList()
                                            .stream()
                                            .map(UUID::fromString)
                                            .toList())))
                            .exists());
        ...
        if (request.getPageable().getSort().equalsIgnoreCase("title")) {
            query.orderBy(new OrderSpecifier<>(order, post.title, OrderSpecifier.NullHandling.NullsFirst));
        } else {
            query.orderBy(new OrderSpecifier<>(order, post.postId, OrderSpecifier.NullHandling.NullsFirst));
        }
