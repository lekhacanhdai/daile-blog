...
public class PostGrpcServiceImpl implements PostGrpcService {
  ...
  public MListPostResponse listPost(MListPostRequest request) {
    ...
    var userIds =
        pagePosts.getItems().stream().map(PostEntity::getUserId).collect(Collectors.toSet());

    var userMap =
        userRepository.findAllById(userIds).stream()
            .collect(Collectors.toMap(CdcUserEntity::getUserId, e -> e));

    return MListPostResponse.newBuilder()
        .setSuccess(true)
        .setData(
            MListPostResponse.Data.newBuilder()
                .addAllPosts(postGrpcMapper.toGrpc(pagePosts.getItems(), userMap))
                .setPageable(
                    PageableUtils.normalize(request.getPageable(), pagePosts.getTotalElement()))
                .build())
        .build();
  }
  ...
  public MCreatePostResponse createPost(MCreatePostRequest request) {
    ...
    return MCreatePostResponse.newBuilder()
        .setSuccess(true)
        .setData(IdResponse.newBuilder().setId(savedPost.getPostId().toString()).build())
        .build();
  }
  ...
  private List<PostTagEntity> toPostTag(MCreatePostRequest request, PostEntity post) {
    var tags =
        tagRepository.findAllById(request.getTagIdsList().stream().map(UUID::fromString).toList());
    return tags.stream()
        .map(
            t -> {
